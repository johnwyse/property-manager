...
The application serves as the main entry point, with unauthenticated users automatically directed to the login page.
...
📦capstone
 ...
 ┃ ┣ 📜urls.py - main URL configuration, routes to property app
 ...
 ┃ ┣ 📜tests.py - testing for valid users, messages, and units
 ┃ ┣ 📜urls.py - URL paths for all property app routes
...
The index route serves as the main entry point for the application. Unauthenticated users are automatically shown the login page. After successful login, managers see all their properties displayed, while tenants see their unit info along with dropdown forms to report new issues or send messages to their property manager.
...
5. Run:
   ...
   ```
6. Visit the app locally in your browser at `http://localhost:8000` 
...
**Note**: The application now serves as the main entry point. Unauthenticated users will automatically be directed to the login page.
...
- Multiple tenants at each unit: As of now only one tenant can be attached to each unit. This could be expanded since multiple people could live in each unit. This would likely require another many-to-many model.
